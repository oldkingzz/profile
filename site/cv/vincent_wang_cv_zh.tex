...
\begin{cvitems}
  ...
  \item 处理 7 自由度机械臂 IK 多解造成的异常运动，并将回放精度问题定位到 IK 所依赖的错误 URDF。
  \item 深度参与公司自研 VLA 模型的前期训练，负责训练实现与配置、分布式运行、checkpoint 管理、初始消融和故障诊断；定位 gripper 维度输出 contract 错误与 GPU ECC 硬件故障。
  \item 独立定义并实现面向 flow-matching VLA 蒸馏的 Path-OPD，保存 student inner solver chain，并在梯度隔离下查询 frozen Teacher velocity。在 occupancy、Teacher query、训练和评测预算匹配的条件下，闭环成功率明显优于 endpoint DAgger；正在准备 ICLR 2027 投稿。
\end{cvitems}

\cventry{Wuji Tech \textbar{} 机器人控制实习生}{2024 年 7 月 - 9 月}{灵巧手遥操作与底层电机控制}
\begin{cvitems}
  \item 在 MATLAB/Simulink 中将不稳定的阻抗控制环重构为带力矩反馈的导纳控制器，用于灵巧手遥操作。
  \item 开发 Linux C++ 上位机，实现约 30 个电机的可靠 CAN 通信与底层控制。
\end{cvitems}

\cvsection{代表性项目、研究与领导经历}
\cventry{University of Pennsylvania \textbar{} 系统与机器人项目}{2025 - 2026}{嵌入式自主系统与操作系统工程}
\begin{cvitems}
  \item 在 ESP32/FreeRTOS 上构建自主移动机器人闭环，集成传感、Vive/EKF 定位、A* 规划、底盘控制、任务状态机与系统验证。
  \item 使用 C 实现并调试 PennOS，覆盖进程、调度、系统调用、信号、文件系统与 Shell。
\end{cvitems}
...
\cvskill{VLA 与训练}{PyTorch、分布式训练、VLA 后训练、flow matching、online policy distillation、action-head 微调、RLinf、Pi0.5}
\cvskill{机器人数据与评测}{LeRobot、多相机数据、时间戳同步、轨迹筛选、闭环评测}
\cvskill{研究与实验运维}{Matched 实验、checkpoint 管理、训练诊断、CUDA、Teacher NFE 与 wall-clock 核算}
